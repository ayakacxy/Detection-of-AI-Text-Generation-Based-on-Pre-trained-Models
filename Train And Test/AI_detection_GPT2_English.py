...
class TextDataset(Dataset):
    def __init__(self, texts, labels, tokenizer, max_len=192):
        self.texts = [preprocess_text(text) for text in texts if preprocess_text(text)]  # 确保文本不为空
        self.labels = [label for text, label in zip(texts, labels) if preprocess_text(text)]  # 确保对应标签同步过滤
...
def load_model(path, num_labels=2):
    model = GPT2LMHeadModel.from_pretrained('/home/group2024-detect4/.vscode-server/data/User/GPT2_English')
...
# 使用提供的最佳超参数
best_params = {'batch_size': 32, 'lr': 2e-05, 'max_len': 256}

# 加载数据
file_path = '/home/group2024-detect4/.vscode-server/data/User/HC3_2.jsonl'
...
# 初始化分词器
tokenizer = GPT2Tokenizer.from_pretrained('/home/group2024-detect4/.vscode-server/data/User/GPT2_English')
...
# 初始化模型
gpt2_model = GPT2LMHeadModel.from_pretrained('/home/group2024-detect4/.vscode-server/data/User/GPT2_English')
...
# 保存模型
save_model(model, '/home/group2024-detect4/.vscode-server/data/User/Detection_English_GPT2.pth')
