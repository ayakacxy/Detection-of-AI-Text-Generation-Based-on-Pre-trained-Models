...
def train_model(model, train_loader, val_loader, optimizer, scheduler, scaler, num_epochs=6, patience=2, delta=0.01):
...
def objective(trial):
    ...
    model.classifier.apply(initialize_weights)  # 手动初始化分类层的权重
    model = model.to('cuda')
...
# 保存模型
save_model(model, '/home/group2024-detect4/.vscode-server/data/User/Detection_Chinese_BERT_2.pth')
